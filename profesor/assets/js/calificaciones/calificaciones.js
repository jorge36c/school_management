...
document.addEventListener('DOMContentLoaded', function() {
    ...
    tabButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            // Código específico para la pestaña de asistencia
            if (viewType === 'asistencia') {
                if (typeof asistenciaModule !== 'undefined') {
                    if (!asistenciaModule.initialized) {
                        asistenciaModule.init({
                            asignacionId: document.getElementById('asignacion_id')?.value,
                            esMultigrado: document.getElementById('es_multigrado')?.value === '1',
                            nivel: document.getElementById('nivel')?.value,
                            sedeId: document.getElementById('sede_id')?.value,
                            materiaId: document.getElementById('materia_id')?.value
                        });
                        asistenciaModule.initialized = true;
                    }
                }
            }
        });
