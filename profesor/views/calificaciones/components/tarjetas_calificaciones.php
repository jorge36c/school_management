<div class=                    <?php if (!empty($estudiante['foto_url'])): ?>
                ...
                <div class="student-card-info">
                    <h3 class="student-name" title="<?php echo htmlspecialchars($estudiante['apellidos'] . ', ' . $estudiante['nombres']); ?>">
                        <?php echo htmlspecialchars($estudiante['apellidos'] . ', ' . $estudiante['nombres']); ?>-grid">
    <?php foreach ($estudiantes as $estudiante): 
        ...
        $completitud = CalificacionesHelper::calcularCompletitud($estudiante['calificaciones'], $tipos_notas);
            ...
            <div class="card-header">
                <div class="student-photo">
                    <?php if (!empty($estudiante['foto_url'])): ?>
                        <img src="<?php echo htmlspecialchars($estudiante['foto_url']); ?>" alt="Foto de <?php echo htmlspecialchars($estudiante['nombre']); ?>">
                    <?php else: ?>
                        <?php echo strtoupper(substr($estudiante['nombre'], 0, 1) . substr($estudiante['apellido'], 0, 1)); ?>
                    <?php endif; ?>
                </div>
                <div class="student-info">
                    <h3 class="student-name" title="<?php echo htmlspecialchars($estudiante['apellido'] . ', ' . $estudiante['nombre']); ?>">
                        <?php echo htmlspecialchars($estudiante['apellido'] . ', ' . $estudiante['nombre']); ?>
                    </h3>
                </div>
                <div class="card-grade <?php echo CalificacionesHelper::getColorClase((float)$definitiva); ?>">
                    <?php echo $definitiva; ?>
                </div>
            </div>
            <div class="card-body">
                            ...
                            <?php foreach ($tipos as $tipo): ?>
                                <div class="nota-card-item">
                                    <div class="nota-card-label">
                                        ...
                                        <span class="nota-card-percent"><?php echo $tipo['porcentaje']; ?>%</span>
                                    </div>
                                    <input type="number" 
...
<!-- Botón para guardar en vista de tarjetas -->
<div class="form-actions">
    <button type="button" class="btn-guardar-todas" id="btnGuardarNotasTarjetas">
